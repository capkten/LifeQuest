  ...
  <div class="sects-page">
    ...
    <section v-else class="sect-list" aria-label="宗门比较结果">
      <article v-for="sect in sects" :key="sect.id || sect.sect_key" class="sect-card cultivation-surface" :class="{ 'sect-card--special': sect.kind === 'special' }">
        ...
        <dl class="sect-card__details"><div><dt>类型</dt><dd>{{ kindLabel(sect.kind) }}</dd></div><div><dt>核心传承</dt><dd>{{ sect.core_legacy || '传承资料待服务器返回' }}</dd></div><div><dt>固定 NPC</dt><dd>{{ sect.fixed_npcs?.length ? sect.fixed_npcs.join('、') : '暂无已确认 NPC' }}</dd></div><div><dt>任务偏好</dt><dd>{{ sect.task_preference || '未设定' }}</dd></div><div><dt>入门境界</dt><dd>{{ sect.entry_realm || '服务器确认' }}</dd></div><div><dt>试炼状态</dt><dd>{{ sect.trial_status || '等待服务器确认' }}</dd></div></dl>
        <div class="sect-card__actions"><span v-if="sect.joined" role="status">已加入</span><span v-else>{{ eligibilityMessage(sect) }}</span><button type="button" class="cultivation-action" :disabled="busyId === sect.sect_key || sect.joined || sect.visible !== true || sect.can_join !== true" @click="join(sect)">{{ busyId === sect.sect_key ? '确认中...' : '加入宗门' }}</button></div>
      </article>
    </section>
  </div>
</template>
