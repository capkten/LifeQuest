...
async function load() { loading.value = true; error.value = null; try { const [response, npcResponse] = await Promise.all([cultivationService.getSects(filters), cultivationService.getNpcs()]); sects.value = Array.isArray(response) ? response.filter((sect) => sect.visible === true) : []; relationship.value = npcResponse || { recently_met: [] } } catch (requestError) { error.value = requestError } finally { loading.value = false } }
