<template>
  <div class="sects-page">
    ...
    <p v-if="!loading && !error && !sects.length" class="cultivation-state">暂无符合条件的宗门。</p>
    <section v-if="!loading && !error && sects.length" class="sect-list" aria-label="宗门比较结果">
      <article v-for="sect in sects" :key="sect.id || sect.sect_key" class="sect-card cultivation-surface" :class="{ 'sect-card--special': sect.kind === 'special' }">
        ...
        <div class="sect-card__actions"><span v-if="sect.joined" role="status">已加入</span><span v-else>{{ eligibilityMessage(sect) }}</span><div v-if="!sect.joined" class="sect-card__action-group"><button type="button" class="cultivation-action" :disabled="busyId !== null" @click="contactMessenger(sect)">{{ busyId === sect.sect_key ? '处理中...' : sect.messenger_contacted ? '已联系使者' : '联系使者' }}</button><button type="button" class="cultivation-action" :disabled="busyId !== null" @click="completeTrial(sect)">{{ sect.trial_confirmed ? '试炼已完成' : accessFor(sect) ? '提交试炼' : '查看试炼' }}</button><button type="button" class="cultivation-action" :disabled="busyId !== null" @click="join(sect)">{{ busyId === sect.sect_key ? '确认中...' : '加入宗门' }}</button></div></div>
        <section v-if="activeTrialSectKey === sect.sect_key && accessFor(sect)" class="sect-trial" aria-label="宗门试炼目标">
          ...
          <ol class="sect-trial__objectives">
            <li v-for="(objective, objectiveKey) in accessFor(sect).objectives" :key="objectiveKey" :class="{ 'sect-trial__objective--done': objective.completed }">
              ...
              <button v-if="!objective.completed" type="button" class="cultivation-action cultivation-action--compact" :disabled="busyId !== null" @click="markTrialObjective(sect, objectiveKey)">{{ busyId === sect.sect_key ? '处理中...' : '标记完成' }}</button>
...
function explainBlocked(message) { showError(message); activeTrialSectKey.value = null }
...
async function loadTrialAccess(sect) {
  ...
  try {
    ...
    if (!Object.values(access.objectives || {}).some((objective) => objective.required && !objective.completed)) {
      showSuccess('试炼目标已满足，可以提交入门试炼。')
    } else {
      showError('请完成下方必需目标，再提交入门试炼。')
    }
  } catch (requestError) {
