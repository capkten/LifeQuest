  ...
  <div class="techniques-page">
    ...
    <template v-else>
      ...
      <section class="techniques-library cultivation-surface" aria-labelledby="technique-library-title">
        ...
        <ul v-else class="technique-list">
          <li v-for="technique in techniques" :key="technique.id" :class="{ 'technique-card--conflict': conflicts(technique).length, 'technique-card--locked': !technique.learned || technique.realm_confirmed === false }">
            ...
            <dl><div><dt>功法类型</dt><dd>{{ technique.technique_type_label || labelTechniqueType(technique.technique_type) }}</dd></div><div><dt>需要境界</dt><dd>{{ technique.required_realm_label || labelRealm(technique.required_realm) }}</dd></div><div><dt>灵石</dt><dd>{{ technique.spirit_stone_cost }}</dd></div><div><dt>占用格数</dt><dd>{{ technique.slot_count }}</dd></div><div><dt>状态</dt><dd>{{ statusLabel(technique) }}</dd></div></dl>
      ...
      <section v-if="selectedSlot" class="purchase-panel cultivation-surface" aria-labelledby="purchase-title">
        ...
        <p>当前灵石：{{ spiritStones }}</p><p>目标格子：{{ slotLabel(selectedSlot.slot_type) }}第 {{ selectedSlot.slot_index + 1 }} 格</p><p>需要境界：{{ selectedSlot.required_realm_label || labelRealm(selectedSlot.required_realm) }} · 灵石：{{ selectedSlot.price }} · 购买后余额：{{ selectedSlot.balance }}</p>
...
import { labelRealm, labelSlotType, labelTechniqueType } from '../utils/displayLabels'
...
function statusLabel(technique) { if (technique.status_label) return technique.status_label; if (!technique.learned) return '未学会'; if (technique.realm_confirmed === false) return '境界不足'; if (conflicts(technique).length) return '⚠ 冲突'; if (!hasRequiredSlots(technique)) return '连续格子不足'; return '可配置' }
