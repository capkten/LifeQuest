<template>
  <section class="cultivation-tribulation-probability stitch-surface" aria-labelledby="tribulation-probability-title" :aria-busy="operationBusy">
    ...
    <div v-else-if="error" class="cultivation-state cultivation-state--error" role="alert">
      ...
      <button type="button" class="cultivation-action" :disabled="loading || operationBusy" @click="$emit('retry')">Retry</button>
    </div>
    <template v-else-if="preview">
      ...
      <button type="button" class="cultivation-action" :disabled="Boolean(preview.cooldown_until) || loading || operationBusy" @click="$emit('attempt')">
        {{ operationBusy ? 'Attempting...' : 'Attempt tribulation' }}
      </button>
    </template>
  </section>
</template>
