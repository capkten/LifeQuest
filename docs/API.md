...
MCP 访问令牌用于连接 LifeQuest MCP 服务，不等同于登录接口返回的 Web JWT。请先登录 Web 端，进入 `Profile` 页面（路径：`/profile`）的个人设置区域，在“MCP 访问凭证”中填写名称和有效期（1–365 天，默认 90 天）并创建令牌。
...
撤销当前用户的指定令牌。Profile 页面令牌列表中的“撤销”操作调用此接口；新连接以及重新认证时使用已撤销令牌的请求会被拒绝。已有会话的客户端需要在撤销后断开并重新连接。重复撤销保持幂等，撤销响应只返回元数据，不返回完整令牌。
